  ...
  // ------------------------------------------------------
    {
      ...
      for(i in 1:n){
        // Hazard function
        haz[i] =  nu * Time[i]^( nu-1) * exp(gamma + Alpha * inv_logit(betas1[1] + bi[i,1] + (betas1[2] + bi[i,2])*Time[i]) );
        
        // Hazard function evaluated at Gauss-Legendre quadrature integration points
        for(j in 1:K){
          cumHazK[i,j] =  nu * (Time[i]/2*(xk[j]+1))^(nu-1) * exp( gamma + Alpha * inv_logit(betas1[1] + bi[i,1] + (betas1[2] + bi[i,2])*Time[i]/2*(xk[j]+1) ) );
        }
